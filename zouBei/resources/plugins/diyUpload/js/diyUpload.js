...
(function( $ ) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		var $box = $('#box');
		var file_id = file.id;
		var $parentFileBox = $box.find('.parentFileBox');
		...
		if ( $parentFileBox.length <= 0 ) {
			...
			// $fileInput.after( div );
			$box.append( div );
			$parentFileBox = $box.find('.parentFileBox');
		...
		//父容器宽度;
		$boxWidth = $box.width();
		$leftWidth = ( $boxWidth - 20) % 180;
		$parentFileBox.width( $boxWidth - $leftWidth);
		$box.width($boxWidth - $leftWidth);

		// var $width = $('.fileBoxUl>li').length * 175;
